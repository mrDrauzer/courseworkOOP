...
class FileSaver(ABC):
    ...
    Конкретные реализации (JSON, CSV, БД и т.д.) должны наследовать этот класс и реализовывать методы.
    """
    ...
    def get_vacancies(self, criteria: Optional[Dict] = None) -> List[Dict]:
        """
        Получить список вакансий из хранилища, с опциональной фильтрацией по критериям.
        :param criteria: Словарь с критериями фильтрации (например, ключевые слова, диапазон зарплат и т.п.)
