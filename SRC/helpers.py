...
def filter_vacancies_by_keyword(
    vacancies: List[Dict], keywords: List[str]
) -> List[Dict]:
    """
...
def sort_vacancies_by_salary(
    vacancies: List[Dict], descending: bool = True
) -> List[Dict]:
    """
...
def filter_by_salary_range(
    vacancies: List[Dict], min_salary: int = 0, max_salary: int = 1_000_000
) -> List[Dict]:
    """
    Фильтр по диапазону зарплаты
    (берется salary_from либо salary_to по возможности).
    """
    result = []
    for v in vacancies:
